...
def find_dominators(func, print_cfg=False, printop=False):
    # Print dominators at end
...
def build_matrix(tree):
    
    def dfs_build(tree, temp, i):
        temp[i] = True
        for s in tree[i]:
            dfs_build(tree, temp, s)

    matrix = []
    for i in range(0, len(tree)):
        temp = [False]*len(tree)
        dfs_build(tree, temp, i)
        matrix.append(temp)

    return matrix
    # returns: matrix[i][j] = True iff i dominates j  

def build_tree(dominators, printop=False):
    # Print tree at end
