...
from graphutils import to_graph_viz
...
#        print("loop exits", loop_exits)
       ...
       # print(keep_set)
        ...
        # go thru all instructions; if a block inside the scc goes to head, make it go to normal head, else make it go to preheader
        for i in range(0,n):
            if i in scc:
                pass
            else:
                for ii in range(0,len(blocks[i])):
                    if 'labels' in blocks[i][ii]:
                        blocks[i][ii]['labels'] = [gen_preheader if label == blocks[head][0]['label'] else label for label in blocks[i][ii]['labels'] ]
